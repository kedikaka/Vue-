<template>
  ...
  <div class="page-header">
    <h1 class="title">步骤列表</h1>
    <ol class="breadcrumb">
      <li class="active">查看所有的步骤</li>
    </ol>
          ...
          <div class="panel-body">
            <form class="form-horizontal">
              ...
              <div class="form-group">
                <label for="input001" class="col-sm-2 control-label form-label">搜索步骤</label>
                <div class="col-sm-10" style="width:30%">
                  <input type="text" class="form-control">
                  <span id="helpBlock" class="help-block">输入步骤名称，支持模糊搜索。</span>
                </div>
              </div>

              <div class="col-sm-offset-2 col-sm-10">
                <a type="submit" @click="search" class="btn btn-default">搜索</a>
                <router-link :to="{ path: '/step/create'}" class="btn btn-success">新建步骤</router-link>  
              </div>
            </form>
          ...
          <table class="table table-bordered table-striped">
            ...
            <tbody>
              ...
              <tr  v-for="(entity, index) of listData" v-bind:key="index" >
                <td># <b>{{entity.stepName}}</b></td>
                <td>{{entity.manageUserName}}</td>
                <td>{{entity.manageRealName}}</td>
                <td>
                  
                  <div v-html="getStepStatusHtml(entity.stepStatus)"></div>
                </td>
                <td>{{entity.beginDate}}</td>
                <td>{{entity.endDate}}</td>
                <td>
                  <router-link :to="{ path: '/step/update', query:{ stepId: entity.stepId}}" class="btn-xs btn-rounded btn-default">修改</router-link>
                  <router-link :to="{ path: '/step/detail', query:{ stepId: entity.stepId}}" class="btn-xs btn-rounded btn-option3">详情</router-link>  
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  methods: {
    ...
    search() {
      var result_en = ajax
        .get("/restapi/step/search/findByProcessIdOrderByPriorityDesc", {
          processId: this.processId
        })
        .then(response => {
          this.listData = response._embedded.stepEntities;
        });
    }
  }
};
